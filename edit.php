...
<body>
    ...
    $stmt_get = $conn->prepare("SELECT kode_kategori, nama_kategori, deskripsi, status FROM kategori WHERE id_kategori = ?");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($kode)) {
            ...
            $stmt_cek = $conn->prepare("SELECT 1 FROM kategori WHERE kode_kategori = ? AND id_kategori != ? LIMIT 1");
            $stmt_cek->bind_param("si", $kode, $id);
            $stmt_cek->execute();
            $stmt_cek->store_result();
            
            if ($stmt_cek->num_rows > 0) {
                $errors[] = "Kode Kategori sudah dipakai kategori lain.";
            }
            $stmt_cek->close();
        }
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="kode" name="kode" value="<?= htmlspecialchars($kode, ENT_QUOTES, 'UTF-8'); ?>" required placeholder="Contoh: KAT-01">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3"><?= htmlspecialchars($deskripsi, ENT_QUOTES, 'UTF-8'); ?></textarea>
                            </div>
